...
$files = @(
    ...
    @{ Path = Join-Path $projectRoot "papyrus\source\SRSSE_MCM.psc";          Pattern = '(?<=AddTextOption\("Version", ")\d+\.\d+\.\d+'; Replace = $Version },
    @{ Path = Join-Path $projectRoot "docs\readme.txt";                       Pattern = '(?<=Version )\d+\.\d+\.\d+'; Replace = $Version },
